...
def calculate_capex(analysis_period, capex, debt_percent):
    ...
    return round_floats(result)
# def calculate_revenue(analysis_period, production, ppa_price):
#     revenues_per_year = []
#     total_revenue = 0
...
#     return revenues_per_year, total_revenue

# def calculate_opex(analysis_period, production, opex):
#     opex_per_year = []
#     total_opex = 0
...
#     return opex_per_year, total_opex

# def calculate_ebitda(analysis_period, production, ppa_price, opex):
#     revenues_per_year = calculate_revenue(analysis_period, production, ppa_price)
#     opex_per_year = calculate_opex(analysis_period, production, opex)
    
#     ebitda_per_year = []
#     total_ebitda = 0
    
#     for year in range(1, analysis_period + 1):
#         yearly_ebitda = revenues_per_year[year] - opex_per_year[year]
#         ebitda_per_year.append(yearly_ebitda)
#         total_ebitda += yearly_ebitda
    
#     return ebitda_per_year, total_ebitda
...
def operating_activities(analysis_period, production, ppa_price, opex):
    ...
    #     yearly_ebitda = yearly_revenue - yearly_opex
    #     ebitda_per_year.append(yearly_ebitda)
    #     total_ebitda += yearly_ebitda
...
# def cash_flow_available_for_debt_service(analysis_period, capex, debt_percent, production, ppa_price, opex, depreciation_y, interest_rate, tax_rate):
#     capex_info = calculate_capex(analysis_period, capex, debt_percent)
#     total_capex = capex_info["total_capex"]
#     funding = capex_info["funding"]
#     operating_info = operating_activities(analysis_period, production, ppa_price, opex)
#     total_ebitda = operating_info["total_ebitda"]
    
#     cash_flow_after_funding_before_tax = total_ebitda - total_capex - funding
    
#     tax_info = tax_paid_g(analysis_period, capex, debt_percent, production, ppa_price, opex, depreciation_y, interest_rate, tax_rate)
#     total_tax_paid = sum(tax_info["tax_paid_per_year"])
    
#     cash_flow_available_for_debt_service = cash_flow_after_funding_before_tax - total_tax_paid
...
# def debt_service(analysis_period, capex, debt_percent, production, ppa_price, opex, depreciation_y, interest_rate, tax_rate, repayment_years):

#     cash_flow_info = cash_flow_available_for_debt_service(analysis_period, capex, debt_percent, production, ppa_price, opex, depreciation_y, interest_rate, tax_rate)
#     cash_flow_available_for_debt_service_value = cash_flow_info["cash_flow_available_for_debt_service"]
    
#     capex_info = calculate_capex(analysis_period, capex, debt_percent)
#     senior_debt = capex_info["senior debt"]
#     debt_info = calculate_debt(analysis_period, senior_debt, repayment_years)
    
#     interest_expense_total = sum([senior_debt * interest_rate] * analysis_period)
#     principal_payment_total = sum(debt_info["principal_payment"])
#     total_debt_service = interest_expense_total + principal_payment_total
    
#     dscr = cash_flow_available_for_debt_service_value / total_debt_service if total_debt_service > 0 else 0
    
#     cash_flow_available_to_equity = cash_flow_available_for_debt_service_value - total_debt_service
    
#     return {
#         "cash_flow_available_for_debt_service": cash_flow_available_for_debt_service_value,
#         "total_debt_service": total_debt_service,
#         "dscr": dscr,
#         "cash_flow_available_to_equity": cash_flow_available_to_equity
#     }
...
#     # First part: calculate cash flow available for debt service
#     capex_info = calculate_capex(analysis_period, capex, debt_percent)
#     total_capex = capex_info["total_capex"]
#     funding = capex_info["funding"]
#     operating_info = operating_activities(analysis_period, production, ppa_price, opex)
...
#     # Second part: calculate debt service details
#     senior_debt = capex_info["senior_debt"]  
#     debt_info = calculate_debt(analysis_period, repayment_years, capex, debt_percent)
    
#     interest_expense_total = interest_rate * senior_debt * analysis_period
#     principal_payment_total = sum(debt_info["principal_payment"])
#     total_debt_service = interest_expense_total + principal_payment_total
    
#     dscr = cash_flow_available_for_debt_service / total_debt_service if total_debt_service > 0 else 0
    
#     cash_flow_available_to_equity = cash_flow_available_for_debt_service - total_debt_service
    
#     # Return combined information from both cash flow analysis and debt service analysis
#     return {
#         "cash_flow_after_funding_before_tax": cash_flow_after_funding_before_tax,
#         "total_tax_paid": total_tax_paid,
#         "cash_flow_available_for_debt_service": cash_flow_available_for_debt_service,
...
#         "interest_expense_total": interest_expense_total,
#     }
...
#     # Calculate cash flows available to equity
...
#     cash_flows_to_equity = [cash_flow_info['cf_aoe']]  # Year 0 is the initial investment, hence negative
#     cash_flows_to_equity[0] = -cash_flow_info['cf_aoe']  # Adjusting Year 0 to be negative as per the requirement
    
#     # Generate cash flows for subsequent years
#     for year in range(1, analysis_period):
#         cash_flows_to_equity.append(cash_flow_info['cf_aoe'])

#     cumulative_cash_flow_equity = np.cumsum(cash_flows_to_equity)
    
#     # Calculate NPV to Equity
#     npv_to_equity = npf.npv(equity_return, cash_flows_to_equity[1:]) - cash_flows_to_equity[0]
    
#     # Calculate IRR to Equity
#     try:
#         irr_to_equity = npf.irr(cash_flows_to_equity)
#     except:
#         irr_to_equity = "Calculation Error"
...
# def project_return(analysis_period, capex, debt_percent, production, ppa_price, opex, depreciation_y, interest_rate, tax_rate, project_return):
#     # Utilize the existing functions to calculate necessary components
#     capex_info = calculate_capex(analysis_period, capex, debt_percent)
#     operating_info = operating_activities(analysis_period, production, ppa_price, opex)
#     tax_info_ug = tax_paid_ug(analysis_period, capex, debt_percent, production, ppa_price, opex, depreciation_y, tax_rate)
    
#     # Step 1: Cash flow available to project before tax
#     cash_flow_available = [capex_info['yearly_capex'][0] * -1] + operating_info['ebitda_per_year']
    
#     # Step 2: Tax paid (ungeared), year 0 is 0, subsequent years use tax_paid_ug function
#     tax_paid = [0] + tax_info_ug['tax_paid_ug_per_year']
    
#     # Step 3: Cash flow to project
#     cash_flow_to_project = [cash_flow_available[i] - tax_paid[i] for i in range(analysis_period)]
    
#     # Step 4: Cumulative cash flow to project (not directly needed for NPV/IRR calculation)
#     cumulative_cash_flow = np.cumsum(cash_flow_to_project)
    
#     # Step 5: NPV to project
#     npv_project = npf.npv(project_return, cash_flow_to_project)
    
#     # Step 6: IRR to project
#     irr_project = npf.irr(cash_flow_to_project)
    
#     return {
#         "Cash Flow Available to Project": cash_flow_available,
#         "Tax Paid (Ungeared)": tax_paid,
#         "Cash Flow to Project": cash_flow_to_project,
#         "ccp": cumulative_cash_flow,
#         "npvp": npv_project,
#         "irrp": irr_project
#     }
...
#     # Step 4: Cumulative cash flow to project
#     cumulative_cash_flow = np.cumsum(cash_flow_to_project).tolist()  # Convert Numpy array to list
...
#         "Tax Paid (Ungeared)": tax_paid,
#         "Cash Flow to Project": cash_flow_to_project,
#         "ccp": cumulative_cash_flow,  # Now a list
#         "npvp": npv_project,
#         "irrp": irr_project
#     }
